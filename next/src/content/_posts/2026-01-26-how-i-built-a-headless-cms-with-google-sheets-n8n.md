...
featured: true
categories: [Automation, n8n, Blogging]
description: "How I built a lightweight headless CMS with Google Sheets and n8n for my Jekyll site, including the bugs that broke the workflow and the fixes that made it publish cleanly."
---
...
My workflow kept creating folders instead of files. I'd ask for `_posts/my-file.md`, and GitHub would create a folder named `my-file.md/`. The culprit was a hidden `\n` newline character at the end of my file path formula. Automation takes you literally, so if you tell it to put a newline at the end of a filename, it interprets that as a directory structure.

### 2. The Double Equals Disaster

I spent an hour fighting a bug where n8n was creating folders named `==_posts` in my repository. It turned out I had a typo in my n8n Expression. I wrote:
...
That extra equals sign was treated as text, so n8n literally sent a file path starting with equals signs to GitHub. Always check your raw expressions.

### 3. The "SHA" Error

When testing, I kept getting a `422 Unprocessable Entity` error saying "SHA wasn't supplied." Translation: "This file already exists." The GitHub API protects you from accidentally overwriting files. If you use the "Create" operation on a file that already exists, it blocks you. I learned to either delete the old file first or version my titles, like "My Post V2", during testing.

## The Frontend Logic

Getting the file into GitHub was step one. Step two was making it show up. My site is static HTML and CSS, but Jekyll uses Liquid tags to make it dynamic. I added this loop to my homepage to fetch the data automatically:
